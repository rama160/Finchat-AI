...
class _ChatScreenState extends State<ChatScreen> {
      ...
      if (_messages.isEmpty) {
        _messages = [
          ChatMessageModel(
            role: 'assistant',
            content:
                'Halo! Saya Finchat. Kamu bisa mencatat beberapa transaksi sekaligus, misalnya:\n\n'
  ...
  Future<void> _handleSend() async {
    ...
    try {
      ...
      // 2. Pertanyaan tentang total transaksi yang bisa dihitung dari SQLite
      // dijawab lokal terlebih dahulu. Fitur ini tidak memerlukan API key.
      ...
      final localTotalReply = _tryAnswerLocalTotalQuery(text, monthTx);
      if (localTotalReply != null) {
        await _addAssistantMessage(localTotalReply);
        return;
      }

      // 3. Jalur AI: pertanyaan finansial atau kalimat transaksi yang kompleks.
      ...
      summary['transactions'] = monthTx.map((tx) => {
        'id': tx.id,
  ...
  String? _tryAnswerLocalTotalQuery(
      String text, List<TransactionModel> transactions) {
    final lower = text.toLowerCase().trim();
    if (!lower.contains('total') && !lower.contains('jumlah')) return null;

    final cleaned = lower
        .replaceAll(RegExp(r'\bberapa\b'), ' ')
        .replaceAll(RegExp(r'\btotal\b'), ' ')
        .replaceAll(RegExp(r'\bjumlah\b'), ' ')
        .replaceAll(RegExp(r'\bbeli\b'), ' ')
        .replaceAll(RegExp(r'\bpembelian\b'), ' ')
        .replaceAll(RegExp(r'\bpengeluaran\b'), ' ')
        .replaceAll(RegExp(r'\bbelanja\b'), ' ')
        .replaceAll(RegExp(r'\buntuk\b'), ' ')
        .replaceAll(RegExp(r'\bdari\b'), ' ')
        .replaceAll(RegExp(r'\bdi\b'), ' ')
        .replaceAll(RegExp(r'\bpada\b'), ' ')
        .replaceAll(RegExp(r'\bini\b'), ' ')
        .replaceAll(RegExp(r'\bbulan\b'), ' ')
        .replaceAll(RegExp(r'\bbulan\s+ini\b'), ' ')
    ...
    final matches = transactions.where((tx) {
      ...
      final haystack =
          '${tx.description} ${tx.merchant} ${tx.category}'.toLowerCase();
      return terms.every(haystack.contains);
    }).toList();

    if (matches.isEmpty) return null;

    final total = matches.fold<double>(0, (sum, tx) => sum + tx.amount);
    final currency = NumberFormat.currency(
      locale: 'id_ID',
      symbol: 'Rp',
      decimalDigits: 0,
    );
    final detail = matches.length == 1
        ? matches.first.description
        : '${matches.length} transaksi';
    return 'Total pengeluaran yang cocok dengan "$text" adalah ${currency.format(total)} ($detail).';
  }
  ...
  String _friendlyError(Object error) {
    ...
    if (message.contains('Failed host lookup') ||
      ...
      return 'Koneksi internet ke Gemini gagal. Periksa koneksi internet dan coba lagi.';
    }
