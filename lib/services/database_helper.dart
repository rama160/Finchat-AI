...
class DatabaseHelper {
  ...
  static final ValueNotifier<int> transactionChanges =
      ValueNotifier<int>(0);
  ...
  Future _createDB(Database db, int version) async {
    ...
    await _createPendingReceiptsTable(db);
  }

  // Ditambahkan untuk mendukung fitur baru (Chat AI, Laporan,
  // Sinkronisasi Google Sheets) TANPA mengubah tabel/kolom
  ...
  Future _createPendingReceiptsTable(Database db) async {
    await db.execute('''
  ...
  // untuk mendukung Dashboard, Laporan, Chat AI, dan Sinkronisasi
  // Google Sheets.
  // =====================================================================
  ...
  Future<int> updateTransaction(
    ...
    if (result > 0) {
      // Edit harus masuk antrean sinkronisasi kembali agar perubahan
      // di Sheets menggantikan baris lama, bukan dianggap sudah
      // tersinkron.
      await db.update(
        'transactions',
        {'is_synced': 0},
  ...
  Future<void> restoreDeletedTransactionIds(
    List<int> ids,
  ) async {
  ...
  Future<List<int>> getDeletedTransactionIds() async {
    ...
    return rows
        .map(
          (row) => (row['id'] as num).toInt(),
        )
        .toList();
  }

  Future<void> clearDeletedTransactionIds(
    List<int> ids,
  ) async {
    if (ids.isEmpty) return;
  ...
  /// breakdown per kategori dalam sebuah daftar transaksi.
  ...
  Future<int> insertPendingReceipt({
    ...
    return db.insert(
      'pending_receipts',
      {
        'image_data': Uint8List.fromList(imageData),
        'mime_type': mimeType,
        'created_at':
  ...
  Future<int> deletePendingReceipt(
    int id,
  ) async {
    final db = await instance.database;
  ...
  Future<int> updatePendingReceiptError(
    int id,
    String message,
  ) async {
    final db = await instance.database;
  ...
  Future<int> getPendingReceiptCount() async {
    ...
    final result = await db.rawQuery(
      'SELECT COUNT(*) AS count FROM pending_receipts',
    );
