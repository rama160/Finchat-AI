...
class DatabaseHelper {
  ...
  Future<int> markAsSynced(int id) async {
    ...
    if (result > 0) transactionChanges.value++;
    return result;
  }
  ...
  Future<int> insertPendingReceipt({
    ...
    final db = await instance.database;
    return db.insert('pending_receipts', {
  ...
  Future<List<Map<String, dynamic>>> getPendingReceipts() async {
    final db = await instance.database;
    return db.query('pending_receipts', orderBy: 'id ASC');
  }

  Future<int> deletePendingReceipt(int id) async {
    final db = await instance.database;
    return db.delete('pending_receipts', where: 'id = ?', whereArgs: [id]);
  }

  Future<int> updatePendingReceiptError(int id, String message) async {
    final db = await instance.database;
    return db.update(
      'pending_receipts',
      {'last_error': message},
  ...
  Future<int> getPendingReceiptCount() async {
    final db = await instance.database;
    final result = await db.rawQuery('SELECT COUNT(*) AS count FROM pending_receipts');
