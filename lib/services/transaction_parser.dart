class TransactionParser {
  ...
  static List<String> splitTransactions(String input) {
    ...
    final separated = protectedInput
        .split(RegExp(r'[,;\n]+'))
        .map((item) => item.replaceAll('<DECIMAL>', ',').trim())
        .where((item) => item.isNotEmpty)
        .toList();

    // Speech-to-text sering menghilangkan koma:
    // "beli nasi 25 ribu beli rokok 30 ribu beli kopi 10 ribu".
    // Jika satu segmen memiliki beberapa nominal, pecah setelah setiap
    // nominal agar voice input memiliki perilaku yang sama dengan input teks.
    final result = <String>[];
    final amountPattern = RegExp(
      r'(?:rp\.?\s*)?(?:\d{1,3}(?:[. ]\d{3})+|\d+)(?:[,.]\d+)?\s*(?:juta|jt|ribu|rb|k)?',
      caseSensitive: false,
    );

    for (final rawSegment in separated) {
      final segment = rawSegment.replaceAll('<DECIMAL>', ',');
      final matches = amountPattern.allMatches(segment).toList();
      if (matches.length <= 1) {
        result.add(segment);
        continue;
      }
      ...
      for (final match in matches) {
        ...
        final part = segment.substring(cursor, end).trim();
        if (part.isNotEmpty) result.add(part);
        cursor = end;
      }
      if (cursor < segment.length) {
        // Sisa teks tanpa nominal digabung ke transaksi sebelumnya hanya jika
        // berupa keterangan/merchant, bukan transaksi baru.
        final tail = segment.substring(cursor).trim();
        if (tail.isNotEmpty && result.isNotEmpty) {
          result[result.length - 1] = '${result.last} $tail'.trim();
        }
      }
    }
  ...
  static String detectCategory(
    ...
    if (type == 'income') {
      if (text.contains('gaji') || text.contains('gajian')) return 'Gaji';
      if (text.contains('bonus')) return 'Bonus';
      if (text.contains('pemasukan') || text.contains('pendapatan')) {
