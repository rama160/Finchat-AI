...
/// tidak dikembangkan, sekaligus memakai model Gemini 3.6 Flash yang stabil untuk produksi.
class GeminiService {
  ...
  static Future<Map<String, dynamic>> processChatMessage({
    ...
    final prompt = '''
Kamu adalah Finchat, asisten keuangan pribadi yang ramah dan ringkas.
...
Daftar transaksi bulan berjalan (gunakan data ini untuk pertanyaan spesifik seperti
'mana pengeluaran rokok', 'berapa total rokok country', atau transaksi pada tanggal tertentu):
${jsonEncode(financeContext['transactions'] ?? [])}
...
Aturan jenis transaksi:
- kata pemasukan, gaji, gajian, bonus, pendapatan, uang masuk, menerima, dibayar, jualan, profit = income/pemasukan.
- jangan pernah mengubah pemasukan menjadi expense hanya karena nominalnya disebut tanpa kata "pengeluaran".
- jika pengguna menulis "pemasukan 1 juta", hasilnya harus type "income".
...
''';
  ...
  /// OCR/ekstraksi struk. Setiap item pada struk dikembalikan sebagai
  /// transaksi terpisah; subtotal, pajak, dan grand total tidak disimpan
  ...
  static Future<List<Map<String, dynamic>>> parseReceipt({
    ...
    const prompt = '''
Analisis foto struk belanja ini untuk pencatatan keuangan.
Ekstrak SEMUA ITEM BARANG/JASA yang benar-benar dibeli sebagai transaksi terpisah.
Jangan menggabungkan semua item menjadi grand total.
Jangan membuat transaksi untuk SUBTOTAL, TOTAL, GRAND TOTAL, PAJAK/PPN,
BIAYA LAYANAN, UANG KEMBALI, atau BARANG YANG DIBATALKAN.
Jika ada kuantitas x harga satuan, gunakan nilai total untuk baris item tersebut.
Jika ada diskon khusus item, gunakan nilai setelah diskon bila terlihat jelas.
Jika hanya total yang terbaca dan item tidak dapat dibaca, boleh membuat SATU transaksi
dengan deskripsi "Total struk" sebagai fallback agar data tidak hilang.
...
Balas HANYA JSON valid dengan format:
{
  "transactions": [
    {
      "type": "expense",
      "category": "Makanan|Rokok|Transportasi|Belanja|Tagihan|Pulsa & Data|Kesehatan|Hiburan|Pendidikan|Rumah Tangga|Lainnya",
      "description": "nama item yang dibeli",
      "amount": angka_nominal_item,
...
Prioritas: satu baris/item = satu transaksi. Jangan gunakan grand total sebagai amount
kecuali tidak ada item yang dapat dibaca.
''';
